...
export default function AuthContainer({ initialPanel = 'role', initialMode = 'login', onLoginSuccess, onBackToIntro }) {
  ...
  const handleFarmerSubmit = async (e) => {
    ...
    if (farmerMode === 'login') {
      ...
      try {
        ...
        localStorage.setItem('access_token', data.access);
        localStorage.setItem('refresh_token', data.refresh);
        localStorage.setItem('user', JSON.stringify(data.user || { username: farmerEmail }));
        triggerToast('Login successful — welcome back!');
        setTimeout(() => {
          onLoginSuccess();
        }, 1000);
  ...
  const getStepLabel = () => {
    if (activePanel === 'role') return 'Welcome to GramSamridh';
  ...
  const districtOptions = govState && jurisdictionData[govState] 
    ? Object.keys(jurisdictionData[govState]).filter(dist =>
        dist.toLowerCase().includes(districtSearch.toLowerCase())
      )
    : [];

  const areaData = govState && govDistrict ? jurisdictionData[govState][govDistrict] : null;
      ...
      <div className="app-shell">
        ...
        <section className="hero" aria-hidden="false">
          ...
          <div className="hero__mid">
            <h1 className="slogan">
              ...
              <span className="type-line type-line--3">Satat <em>Vikas.</em></span>
            </h1>
        ...
        <section className="form-panel">
          <div className="auth-card">
            <p className="eyebrow"><span id="stepLabel">{getStepLabel()}</span></p>
            ...
            <div className={`panel ${activePanel === 'role' ? 'active' : ''}`}>
              ...
              <div className="role-grid">
                ...
                <button type="button" className="role-card" onClick={() => {
                  ...
                  <span>
                    <span className="role-card__title" style={{ display: 'block' }}>Farmer access</span>
                    <span className="role-card__desc" style={{ display: 'block' }}>Register land, access advisories and local support</span>
                  </span>
                  <span className="role-card__arrow" aria-hidden="true">→</span>
                </button>
                <button type="button" className="role-card" onClick={() => {
            ...
            <div className={`panel ${activePanel === 'civilian' ? 'active' : ''}`}>
              ...
              <p className="sub">Log in or create your GramSamridh account.</p>
              ...
              <form onSubmit={handleCivilianSubmit} className={civilianMode === 'register' ? 'create-mode' : ''} noValidate>
                {civilianMode === 'login' && (
                  <div style={{
                    ...
                    <small style={{ display: 'block', marginTop: '4px', color: '#997305' }}>Use password <code>citizen123</code> to log in.</small>
                  </div>
                )}
                ...
                <div className="field">
                  ...
                  <div className="password-field">
                    ...
                    <button 
                      ...
                      onClick={() => setShowCivilianPassword(!showCivilianPassword)}
                    >
              ...
              <p className="fine-print">By continuing you agree to GramSamridh's Terms &amp; Privacy Policy.</p>
            </div>

            {/* PANEL 3: FARMER LOGIN / SIGNUP */}
            <div className={`panel ${activePanel === 'farmer' ? 'active' : ''}`}>
              <button type="button" className="back-btn" onClick={() => setActivePanel('role')}>← Back</button>
              <h2>Farmer access</h2>
              <p className="sub">Log in or create your GramSamridh account.</p>

              <div className="auth-mode-toggle" role="tablist" aria-label="Farmer access mode">
                <button 
                  type="button" 
                  className={farmerMode === 'login' ? 'active' : ''} 
                  onClick={() => { setFarmerMode('login'); setFarmerError(''); }}
                  role="tab" 
                ...
                <button 
                  ...
                  onClick={() => { setFarmerMode('register'); setFarmerError(''); }}
                  role="tab" 
              ...
              <form onSubmit={handleFarmerSubmit} className={farmerMode === 'register' ? 'create-mode' : ''} noValidate>
                {farmerMode === 'login' && (
                  <div style={{
                    background: '#fcf8ec',
                    border: '1px solid #f2e1b8',
                ...
                {farmerError && <p className="form-message" role="alert">{farmerError}</p>}
              </form>
              <p className="fine-print">By continuing you agree to GramSamridh's Terms &amp; Privacy Policy.</p>
            </div>
              ...
              <form onSubmit={handleGovSubmit} noValidate>
                ...
                <div className="field autocomplete">
                  ...
                  {showStateList && stateOptions.length > 0 && (
                    <ul className="autocomplete-list visible" role="listbox">
                      {stateOptions.map(st => (
                        <li key={st} onClick={() => handleStateSelect(st)} role="option">
                          {st}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="field autocomplete">
                  ...
                  <input 
                    ...
                    disabled={!govState}
                    placeholder={govState ? "Start typing your district" : "Select a state first"} 
                    required 
                    autoComplete="off"
                  />
                  {showDistrictList && districtOptions.length > 0 && (
                    <ul className="autocomplete-list visible" role="listbox">
                      {districtOptions.map(dist => (
                        <li key={dist} onClick={() => handleDistrictSelect(dist)} role="option">
                          {dist}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div id="jurisdictionFields">
                    <div className="field">
                      ...
                      <select id="block" value={govBlock} onChange={(e) => setGovBlock(e.target.value)} required>
                        ...
                        {areaData.rural.blocks.map(b => <option key={b} value={b}>{b}</option>)}
                      </select>
                    </div>
                    <div className="field">
                      ...
                      <select id="ruralUnit" value={govLocalBody} onChange={(e) => setGovLocalBody(e.target.value)} required>
                        ...
                        {areaData.rural.localBodies.map(l => <option key={l} value={l}>{l}</option>)}
                      </select>
                    </div>
                    <div className="field">
                  ...
                  <div id="jurisdictionFields">
                    <div className="field">
                      ...
                      <select id="ulb" value={govULB} onChange={(e) => setGovULB(e.target.value)} required>
                        ...
                        {areaData.urban.ulbs.map(u => <option key={u} value={u}>{u}</option>)}
                      </select>
                    </div>
                    <div className="field">
                      ...
                      <select id="urbanWard" value={govWard} onChange={(e) => setGovWard(e.target.value)} required>
                        <option value="">Select ward number</option>
                        {areaData.urban.wards.map(w => <option key={w} value={w}>{w}</option>)}
                      </select>
                    </div>
                  </div>
                )}
